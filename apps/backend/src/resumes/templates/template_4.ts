...
export class ResumePDFTemplate4 {
  ...
  constructor(
    ...
    if (headerImagePath && existsSync(headerImagePath)) {
      ...
      const defaultLogoPath = join(process.cwd(), 'assets', 'template_4.png');
  ...
  private _addContact(doc: any) {
    ...
    const fullText = addressAndPhone + email;

    // Calculate total width of the text
    const totalWidth = doc.widthOfString(fullText);

    // Calculate starting X position to center the text
    const startX = this.marginX + (this.contentWidth - totalWidth) / 2;

    doc.text(addressAndPhone, startX, startY);

    // Draw email with link
    doc
      .fillColor('#0066CC')
      .text(email, startX + doc.widthOfString(addressAndPhone), startY, {
        link: `mailto:${email}`,
        underline: true,
      });
  ...
  private _addSectionHeader(doc: any, title: string) {
    ...
    const backgroundWidth = this.contentWidth;
    const backgroundHeight = headerHeight;
    const backgroundX = this.marginX;
    const backgroundY = startY;

    // Draw solid color background (full-width) with transparency
    doc
      .rect(backgroundX, backgroundY, backgroundWidth, backgroundHeight)
      .fillColor('#2C3E50') // Light gray background
  ...
  private async _drawHeaderImage(doc: any) {
    if (
      ...
      try {
        const imageWidth = this.pageWidth;
        const imageHeight = imageWidth * this.headerImageAspectRatio;
        const x = 0;
        const y1 = 0;
        const y2 = this.pageHeight - imageHeight;
        const centerX = x + imageWidth / 2;
        const centerY = y2 + imageHeight / 2;

        doc.save();
        doc.opacity(0.1);
        doc.image(this.headerImagePath, x, y1, {
          width: imageWidth,
          height: imageHeight,
        ...
        doc.rotate(180);
        doc.translate(-centerX, -centerY);
        doc.image(this.headerImagePath, x, y2, {
          width: imageWidth,
          height: imageHeight,
          fit: [imageWidth, imageHeight],
        });
